...
int main() {
    ...
    { // hand cases: sample
        ...
        tests[0].price = std::vector<int> { 3,7,9 };
        tests[0].rela = std::vector<PII> { {1,2}, {2,3}, {3,1} };
        tests[1].price = std::vector<int> { 5,6,7,8,9 };
        tests[1].rela = std::vector<PII> { {2,1}, {3,1}, {4,1}, {1,5} };
        output_all(tests);
    }
    ...
    for (int top : top_list) { 
        ...
        for (int i = 1; i < layers; i++) {
            RandInt randInt(1, sz);
            std::set<PII> has;
            for (int j = i+1; j < layers; j++) {
                int u, v;
                do {
                    u = i * sz + randInt(), v = j * sz + randInt();
                } while (u == v || has.count(std::make_pair(u, v)));
                has.emplace(u, v);
    ...
    constexpr int nm_list[] = { 50, N };

    // random cases
    for (auto n : nm_list) {
        int m = n;
        for (int top : top_list) {
            std::vector<Testcase> tests(N / n);
    ...
    for (int top : top_list) {
        std::vector<Testcase> tests(N / 1000 * 2);
        for (int i = 0; i < N / 1000 * 2; i++) {
            int n = 50, m = 950;
            if (i % 2) std::swap(n, m);
            auto &[price, rela] = tests[i];
            price = getprice(n, top);
            rela = std::vector<PII>(m);
            RandInt randInt(1, n);
            for (auto &[u, v] : rela) {
